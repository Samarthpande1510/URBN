      ...
      {report ? (
        <div className="px-5 py-4 space-y-4">
          <div className={`rounded-md border px-4 py-3 ${isPass ? "border-green-500/30 bg-green-500/10" : "border-red-500/30 bg-red-500/10"}`}>
            ...
            <p className="text-[11px] text-[#94a3b8] mt-0.5">Submitted {parseServerDate(report.submittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" })}</p>
          </div>
          {report.notes ? (
            <div>
    ...
    <div className="border-b border-[#bfdbfe]/30 px-5 py-4 space-y-2 bg-purple-400/5">
      <div className="flex items-center justify-between">
        <div>
          ...
          {received && <p className="text-[11px] text-[#64748b] mt-0.5">Sample received: {parseServerDate(received).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" })}</p>}
        </div>
        {prev.length > 0 && (
        ...
        return (
          <div key={i} className={`rounded-md border p-3 ${pass ? "border-green-500/20 bg-green-500/5" : "border-red-500/20 bg-red-500/5"}`}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[11px] text-[#94a3b8]">{parseServerDate(r.submittedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" })}</span>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      ...
                      {p.factoryComm?.improvementSampleExpected && p.factoryComm.improvementSampleReceivedAt && (
                        <p className="text-[10px] text-purple-500 mt-0.5">Sample received: {parseServerDate(p.factoryComm.improvementSampleReceivedAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" })}</p>
                      )}
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3 tabular-nums text-[#d97706] whitespace-nowrap text-center">
                      {showHistory ? (
                        <div className="flex flex-col gap-0.5 text-xs">
                          <span> Sample received: {p.sampleGivenDate ? parseServerDate(p.sampleGivenDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" }) : "—"}</span>
                          <span className="text-[#94a3b8]">Report submitted: {p.npdReport?.submittedAt ? fmt(p.npdReport.submittedAt) : "—"}</span>
                        </div>
                      ) : (
                        p.sampleGivenDate
                          ? <span className="text-green-600 text-xs font-medium">✓ {parseServerDate(p.sampleGivenDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "Asia/Kolkata" })}</span>
